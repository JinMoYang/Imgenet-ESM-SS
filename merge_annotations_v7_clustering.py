...
def merge_polygons(polygons: List, method: str) -> List[List[List[float]]]:
    """Merge polygons. Returns list of polygon coordinates (may be multiple for MultiPolygon)."""
    if method == 'union':
...
def process_image_annotations_clustering(
    ...
    """
    Process annotations using IoU-based clustering (v6 algorithm with MultiPolygon support).
    ...
    for cluster_idx, cluster_indices in enumerate(clusters):
        ...
        # Merge polygons - MODIFIED: returns List[List[List[float]]]
        try:
            merged_points_list = merge_polygons(polygons, merge_method)
        except Exception as e:
            print(f"Warning: Failed to merge polygons in cluster {cluster_idx}: {e}")
            rejected_count += 1
            continue
        ...
        # Add merged shape(s)
        # MODIFIED: Handle MultiPolygon - create multiple shapes with same group_id
        group_ids = [obj[2]['group_id'] for obj in cluster_objects]
        group_id_info = ""
        if any(gid is not None for gid in group_ids):
            group_id_info = f" (original groups: {group_ids})"
        
        # If merged result has multiple parts (MultiPolygon), assign a unique group_id
        # to link them together as one logical object
        assigned_group_id = None
        if len(merged_points_list) > 1:
            # Use cluster_idx + 1000 as group_id to avoid conflicts
            assigned_group_id = cluster_idx + 1000
        
        # Create a shape for each part of the merged result
        for part_idx, merged_points in enumerate(merged_points_list):
            merged_shapes.append({
                'label': final_label,
                'points': merged_points,
                'group_id': assigned_group_id,  # Link parts with same group_id
                'shape_type': 'polygon',
                'flags': {},
                ...
                'attributes': {
                    ...
                    'merge_info': f'Cluster {cluster_idx+1}: {num_annotators_in_cluster} annotators, part {part_idx+1}/{len(merged_points_list)}, merged using {merge_method}{group_id_info}',
    ...
    report['num_clusters'] = len(clusters) - rejected_count  # Valid clusters only
    report['num_shapes'] = len(merged_shapes)  # Total shapes (including MultiPolygon parts)
    ...
    merged_annotation = {
        ...
        'mergeMetadata': {
            ...
            'algorithm': 'iou_clustering_v6_multipolygon'
        }
    }
