...
def select_best_annotation(
    ...
    Returns:
        (선택된 shapes, 선택된 annotator_name, pairwise IoU 딕셔너리)
    """
    n = len(candidates)
    if n == 0:
        return [], "", {}
    ...
    return candidates[best_idx][1], candidates[best_idx][2], pairwise_ious
...
def process_image_annotations(
    ...
    for obj_idx, candidates in enumerate(matched_objects):
        selected_shapes, selected_annotator, pairwise_ious = select_best_annotation(candidates)
...
def save_iou_records(iou_records: List[Dict], output_path: str) -> None:
    ...
    # 고정 컬럼 순서
    fixed_cols = ['image_name', 'object_id', 'num_candidates', 'selected_annotator']
    iou_cols = sorted([k for k in all_keys if '-' in k])
    fieldnames = fixed_cols + iou_cols
...
def main(root_folder: str, output_folder: str, iou_output_path: str) -> None:
    ...
    for json_file in json_files:
        process_image_annotations(annotator_folders, json_file, output_folder, iou_records)
