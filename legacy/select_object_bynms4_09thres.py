...
def select_best_annotation(
    ...
    모든 pairwise IoU가 threshold 이상일 때만 평균 IoU로 선택하고,
    ...
    Parameters:
        ...
        nms_iou_threshold: NMS 수행을 위한 최소 IoU 임계값
    ...
    # NMS 수행 조건 확인: 모든 pairwise IoU가 threshold 이상이어야 함
    if iou_values and np.min(iou_values) >= nms_iou_threshold:
        # 조건 충족: NMS 수행 (평균 IoU 기반 선택)
        matching_info['nms_performed'] = True
        ...
        matching_info['selection_reason'] = f"nms_avg_iou={avg_ious[best_idx]:.3f}"
        
        return candidates[best_idx][1], candidates[best_idx][2], pairwise_ious, matching_info
    
    else:
        # 조건 불충족: NMS 수행하지 않고 base 어노테이터의 것 사용
        matching_info['nms_performed'] = False
        ...
        if base_idx is not None:
            matching_info['selection_reason'] = 'base_annotator_fallback'
            return candidates[base_idx][1], candidates[base_idx][2], pairwise_ious, matching_info
        else:
            # base가 없으면 첫 번째 후보 사용
            matching_info['selection_reason'] = 'first_candidate_fallback'
            return candidates[0][1], candidates[0][2], pairwise_ious, matching_info
...
def process_image_annotations(
    ...
    for obj_idx, candidates in enumerate(matched_objects):
        selected_shapes, selected_annotator, pairwise_ious, matching_info = \
            select_best_annotation(candidates, base_annotator, available_annotators, nms_iou_threshold)
        ...
        # NMS 수행 여부 카운트
        if matching_info['nms_performed']:
            nms_performed_count += 1
        else:
            nms_skipped_count += 1
        ...
        record = {
            ...
            'nms_performed': matching_info['nms_performed'],
            ...
            'selection_reason': matching_info.get('selection_reason', 'none'),
        }
        ...
        # NMS를 수행하지 않은 object 기록
        if not matching_info['nms_performed']:
            nms_skip_record = {
                'image_name': image_name,
                'object_id': obj_idx,
                'base_annotator': base_annotator,
                'num_candidates': len(candidates),
                'matched_annotators': ','.join(matching_info['matched_annotators']),
                'nms_skip_reason': matching_info['nms_skip_reason'],
                'min_pairwise_iou': matching_info['min_pairwise_iou'] if matching_info['min_pairwise_iou'] is not None else 'N/A',
                'avg_pairwise_iou': matching_info['avg_pairwise_iou'] if matching_info['avg_pairwise_iou'] is not None else 'N/A',
                'selected_annotator': selected_annotator,
                'selection_reason': matching_info.get('selection_reason', 'none'),
            }
            nms_skip_records.append(nms_skip_record)
    ...
    image_stats = {
        ...
        'base_only': base_only,
        'nms_performed_count': nms_performed_count,
        'nms_skipped_count': nms_skipped_count,
    }
    ...
    result = {
        ...
        "matching_statistics": {
            ...
            "partial_matches": partial_matches,
            "base_only": base_only,
            "nms_performed": nms_performed_count,
            "nms_skipped": nms_skipped_count,
        }
    }
    ...
    print(f"  NMS 수행: {nms_performed_count}, NMS 스킵: {nms_skipped_count}")
...
def save_iou_records(iou_records: List[Dict], output_path: str) -> None:
    ...
    fixed_cols = ['image_name', 'object_id', 'num_candidates', 'num_available_annotators',
                  ...
                  'nms_performed', 'nms_skip_reason', 'min_pairwise_iou', 
                  'max_pairwise_iou', 'avg_pairwise_iou', 'selection_reason']
...
def save_nms_skip_records(nms_skip_records: List[Dict], output_path: str) -> None:
    """
    NMS를 수행하지 않은 object 기록을 CSV 파일로 저장합니다.
    """
    if not nms_skip_records:
        print("경고: NMS skip 기록이 없습니다.")
        return
    
    fieldnames = ['image_name', 'object_id', 'base_annotator', 'num_candidates',
                  'matched_annotators', 'nms_skip_reason', 'min_pairwise_iou',
                  'avg_pairwise_iou', 'selected_annotator', 'selection_reason']
    
    nms_skip_path = output_path.replace('.csv', '_nms_skip.csv')
    with open(nms_skip_path, 'w', newline='', encoding='utf-8') as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames, extrasaction='ignore')
        writer.writeheader()
        writer.writerows(nms_skip_records)
    
    print(f"NMS skip 기록이 저장되었습니다: {nms_skip_path}")
...
def save_exception_records(exception_records: List[Dict], output_path: str) -> None:
    ...
    if stats_records:
        ...
        stats_fieldnames = ['image_name', 'base_annotator', 'num_available_annotators',
                           ...
                           'nms_performed_count', 'nms_skipped_count']
...
def main(root_folder: str, output_folder: str, iou_output_path: str, nms_iou_threshold: float = 0.9) -> None:
    ...
    for json_file in all_json_files:
        ...
        process_image_annotations(
            ...
            iou_records, exception_records, nms_skip_records,
            nms_iou_threshold
        )
    ...
    # NMS skip 기록 저장
    save_nms_skip_records(nms_skip_records, iou_output_path)
    ...
    print(f"NMS skip 기록: {iou_output_path.replace('.csv', '_nms_skip.csv')}")
