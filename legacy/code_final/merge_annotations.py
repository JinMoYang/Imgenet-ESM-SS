        ...
        for j in range(i + 1, n):
            ...
            for idx_i, group_i in enumerate(ann_i['groups']):
                for idx_j, group_j in enumerate(ann_j['groups']):
                    # 레이블이 다르면 스킵
                    if group_i['label'] != group_j['label']:
                        continue
...
def merge_single_file(base_dir: str, filename: str, output_dir: str, 
    ...
    save_voting_results(approved, rejected, output_csv_path)
    
    print(f"완료: {filename} -> {output_json_path}")
    print(f"투표 결과: {output_csv_path}")
    print(f"  승인: {len(approved)}개, 기각: {len(rejected)}개")
...
if __name__ == '__main__':
    # 사용 예시
    base_directory = '/Users/woojin/Documents/AioT/test/sample_images/Imgenet-ESM-SS/batch_inner/batch_1st/batch_1st_validation'  # annotator1, annotator2, annotator3가 있는 디렉토리
    output_directory = '/Users/woojin/Documents/AioT/test/sample_images/Imgenet-ESM-SS/batch_inner/batch_1st/batch_1st_validation/merged_annotations'
