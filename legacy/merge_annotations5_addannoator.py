...
class AnnotationMerger:
    ...
    def parse_description(self, description: str) -> Dict[str, any]:
        ...
        for part in parts:
            ...
            # "merged from" 같은 병합 메타정보는 스킵
            if 'merged' in part.lower() or 'from' in part.lower() or 'annotation' in part.lower():
                continue
            ...
            # 단순 플래그 (ishole)
                ...
                if key and not key.isdigit():  # 숫자만 있는 것은 스킵
                    attributes[key] = True
    ...
    def merge_all(self, output_dir: str = 'merged_annotations'):
        ...
        for image_name, image_annotations in all_annotations.items():
            ...
            # 실제 어노테이터 이름 추출 (JSON의 annotator 필드 우선, 폴더명 fallback)
            actual_annotators = [
                data.get('annotator', folder_name) 
                for folder_name, data in image_annotations.items()
            ]
            ...
            if merged is not None:
                ...
                self.merge_results.append({
                    ...
                    'annotators': ', '.join(actual_annotators)
                })
            else:
                self.merge_results.append({
                    'image': image_name,
    ...
    def save_reports(self, output_path: Path):
        ...
        with open(result_csv, 'w', newline='', encoding='utf-8') as f:
            writer = csv.DictWriter(
                f, 
                fieldnames=['image', 'status', 'n_objects', 'n_annotators', 'annotators']
            )
            writer.writeheader()
...
def main():
    # 설정
    root_directory = '/Users/woojin/Documents/AioT/test/sample_images/Imgenet-ESM-SS/batch_1st/batch_1st_validation'
    iou_threshold = 0.5
    min_annotators = 2
