...
3. Verify that at least 2 annotators marked the same number of objects
   - MODIFIED: If object counts differ, select annotators with matching counts
   - At least 2 annotators must agree on the count
   - Example: [5, 5, 3] → use 2 annotators with 5 objects, exclude the 3rd
...
5. For each matched object:
   ...
   - Vote on description attributes independently (each attribute requires majority)
     * Parse comma-separated attributes (e.g., "ishole, iscrowd=1" → ["ishole", "iscrowd=1"])
     * Each attribute voted separately: include if majority agrees
     * Example: ["ishole, iscrowd=1", "ishole", "iscrowd=1"] → "ishole, iscrowd=1"
...
def process_image_annotations(
    ...
    # MODIFIED: Check if at least 2 annotators have the same number of objects
    # If not all match, select annotators with matching counts (at least 2 required)
    if len(set(object_counts)) != 1:
        # Find counts that appear at least twice
        count_frequency = Counter(object_counts)
        valid_counts = [count for count, freq in count_frequency.items() if freq >= 2]
        
        if not valid_counts:
            # No two annotators agree on count
            report['mask_error'] = True
            report['vote_status'] = f'count_no_match_{object_counts}'
            return None, report
        
        # If multiple counts have 2+ annotators, choose the one with highest frequency
        # (or if tied, choose the larger count)
        selected_count = max(valid_counts, key=lambda c: (count_frequency[c], c))
        
        # Filter to keep only annotators with the selected count
        selected_indices = [i for i, count in enumerate(object_counts) if count == selected_count]
        
        # Update annotations, grouped_shapes, and annotator_names
        annotations = [annotations[i] for i in selected_indices]
        all_grouped_shapes = [all_grouped_shapes[i] for i in selected_indices]
        annotator_names = [annotator_names[i] for i in selected_indices]
        
        # Record which annotators were excluded
        excluded_annotators = [report['annotators'][i] for i in range(len(report['annotators'])) 
                              if i not in selected_indices]
        excluded_counts = [object_counts[i] for i in range(len(object_counts)) 
                          if i not in selected_indices]
        
        num_annotators = len(annotations)
        num_objects = selected_count
        
        report['vote_status'] = f'partial_{num_annotators}of{len(report["annotators"])}'
        report['excluded_annotators'] = excluded_annotators
        report['excluded_counts'] = excluded_counts
        report['num_annotators'] = num_annotators
        report['annotators'] = annotator_names
    else:
    ...
    for match_idx, match_indices in enumerate(matches):
        ...
        # Vote on description attributes (each attribute voted individually)
        # Parse description into attributes (comma-separated)
        all_attributes = []
        for grouped in matched_grouped:
            if grouped['shapes']:
                desc = grouped['shapes'][0].get('description', '').strip()
                if desc:
                    # Split by comma and clean each attribute
        ...
        # Find all unique attributes across all annotators
        unique_attrs = set()
        for attrs in all_attributes:
            unique_attrs.update(attrs)
        
        # Vote on each attribute independently
        # Attribute is included if majority (or all for 2 annotators) voted for it
        final_attrs = []
        if num_annotators == 2:
            # 2 annotators: require both to agree (unanimous)
            for attr in unique_attrs:
                count = sum(1 for attrs in all_attributes if attr in attrs)
                if count == 2:
                    final_attrs.append(attr)
        else:  # num_annotators == 3
            # 3 annotators: require majority (2 or more)
            for attr in unique_attrs:
                count = sum(1 for attrs in all_attributes if attr in attrs)
                if count >= 2:
                    final_attrs.append(attr)
        
        # Sort for consistency and join with comma
        ...
        for i, idx in enumerate(match_indices):
            desc = ''
            if matched_grouped[i]['shapes']:
                desc = matched_grouped[i]['shapes'][0].get('description', '')
    ...
    # Overall vote status (preserve partial status if present)
    if 'partial' not in report['vote_status']:
    ...
    merged_annotation = {
        ...
        'mergeMetadata': {
            'annotators': annotator_names,
            'excludedAnnotators': report['excluded_annotators'],
            'excludedCounts': report['excluded_counts'],
...
def main():
    ...
    print(f"  MODIFIED: If counts differ, uses annotators with matching counts")
    ...
    for image_name, annotations in annotations_by_image.items():
        ...
        if report['excluded_annotators']:
            excluded_info = f" [excluded: {','.join(report['excluded_annotators'])}]"
        print(f"  {status} {image_name}: {report['vote_status']}{excluded_info}")
    ...
    if 'excluded_annotators' in df.columns:
        df['excluded_annotators'] = df['excluded_annotators'].apply(lambda x: ','.join(x) if isinstance(x, list) else '')
    if 'excluded_counts' in df.columns:
        df['excluded_counts'] = df['excluded_counts'].apply(lambda x: ','.join(map(str, x)) if isinstance(x, list) else '')
    ...
    partial_matches = sum(1 for r in reports if 'partial' in r['vote_status'])
    full_matches = len(merged_annotations) - partial_matches
    print(f"    - Full agreement:     {full_matches}")
    print(f"    - Partial match:      {partial_matches}")
...
"""
python merge_annotations_v5.py \
